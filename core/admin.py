...
class OrderAdmin(AdminChartMixin, admin.ModelAdmin):
    ...
    def get_list_chart_data(self, queryset):
        ...
        for month in months_between_dates(earliest, timezone.now()):
            ...
            monthly_count = sum(1 for order in queryset if order.placed_at.year == month.year and order.placed_at.month == month.month)
            order_counts.append(monthly_count)

        return {
            "labels": labels,
            "datasets": [
                {"label": "Orders per Month", "data": order_counts, "backgroundColor": "#79aec8"},
            ],
        }
...
class OrderItemAdmin(AdminChartMixin, admin.ModelAdmin):
    ...
    def get_list_chart_data(self, queryset):
        ...
        for month in months_between_dates(earliest, timezone.now()):
            labels.append(month.strftime("%b %Y"))
            monthly_revenue = sum(oi.purchase_price * oi.quantity for oi in queryset if oi.order.placed_at.year == month.year and oi.order.placed_at.month == month.month)
            revenue.append(float(monthly_revenue))
        return {
            "labels": labels,
            "datasets": [
                {"label": "Revenue per Month", "data": revenue, "backgroundColor": "#36A2EB"},
            ],
        }
...
class BookListingAdmin(AdminChartMixin, admin.ModelAdmin):
    ...
    def get_list_chart_data(self, queryset):
        ...
        return {
            "labels": labels,
            "datasets": [
                {"label": "Book Listings by Condition", "data": counts, "backgroundColor": colors[:len(counts)]},
            ],
        }
